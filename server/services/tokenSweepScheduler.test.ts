...
// serverDb() applies migration 0002 (device_tokens).
...
describe('sweepExpiredAuthRows (LOW-9)', () => {
  ...
  beforeEach(() => {
    serverDb().raw.exec('DELETE FROM device_tokens;')
  })
  ...
  it('deletes only rows whose expires_at is before now', () => {
    ...
    expect(result).toEqual({ deviceTokens: 1 })
  ...
  it('is a no-op when nothing is expired', () => {
    insertDeviceToken('live', FUTURE)
    expect(sweepExpiredAuthRows(NOW)).toEqual({ deviceTokens: 0 })
  })
})
