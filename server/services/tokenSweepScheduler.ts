...
//
// device_tokens carry an ISO-8601 `expires_at` (indexed). Nothing bulk-deletes
// rows past their expiry, so they accumulate unboundedly over a server's
// lifetime. A row past `expires_at` is already dead — verifyDeviceToken
// rejects it — so hard-deleting it is safe and purely hygienic. Mirrors the dbBackup / iptv-tombstone scheduler
// pattern (returns the ScheduledTask so shutdown can .stop() it; failures reach
// the §15 telemetry pipeline, never console-only).
//
// NOT swept: iptv_playlist_tokens live in the IPTV DB and carry their own
// revocation table.
...
/** Delete expired device_tokens. Exported for test. */
export function sweepExpiredAuthRows(nowIso: string = new Date().toISOString()): { deviceTokens: number } {
