...
export function AppleSignInButton({ inviteCode }: { inviteCode?: string }) {
  const { appleSignIn, signInState } = useAuth()
  const [busy, setBusy] = useState(false)
  const [localError, setLocalError] = useState<string | null>(null)

  if (!isAppleSignInConfigured()) return null

  const pending = busy || signInState === 'pending' || signInState === 'opening'
  ...
  const onClick = async () => {
    ...
    try {
      ...
      if (
        ...
        code.includes('user_trigger_new_signin_flow')
      ) {
        setLocalError(null)
  ...
  return (
    <div className="apple-signin">
      ...
      >
        ...
        <span>{pending ? 'Signing in…' : 'Sign in with Apple'}</span>
      </button>
      {localError && (
