...
- Crit 5 BACKEND CONTRACT MET (2026-06-17): the web client reads/writes `/api/media/watch`, and the native tvOS/iOS client (sibling repo, `46c2265 fix(apple): … continue-watching`) consumes the same route. The cross-device sync mechanism is now pinned by test — `resume_state_is_account_scoped_across_devices` (media-core) proves watch state keys on the verified account `sub` via `acting_sub`, never the device, so two device tokens for one account share resume and other accounts stay isolated. Remaining: a live two-device (web + tvOS) round-trip against the deployed NAS as a belt-and-suspenders demo; the contract itself is proven.
