...
3. **Add M3 measurement harnesses:** ~~100-file <5s scan-timing fixture (crit 2)~~ **DONE 2026-06-14 (`65e3fc3`)** — `scan_once_100_file_library_under_5s` now drives the real `scan_once` over a 100-file fixture via an ffprobe stub and asserts `<5s` + full indexing. Remaining: a TMDB match-accuracy eval against the title-scoring matcher (crit 3).
