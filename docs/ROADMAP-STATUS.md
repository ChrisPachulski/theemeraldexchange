...
What genuinely remains: **an actual TestFlight build upload is unconfirmed**, and App-Store-quality polish (Dynamic Type / VoiceOver, iPad adaptive layout, player loading/stall states) is open in the sibling repo. On the backend, the last open measurement bars all closed 2026-06-14: M4 stress/bench (crit-2/3/6 PASS on the NAS — `scripts/m4-stress-bench.sh`), M3 scan-timing (crit-2, `65e3fc3`), and M3 match-accuracy (crit-3 — 96.1% over a 51-case corpus). The backend milestones are now measurement-complete; the remaining work is Apple-side bug-fix/polish/submission — not greenfield.
