...
| D18 | Replace plain SHA-256 key derivation with HKDF-Extract+Expand (RFC 5869). Session cookie key: `HKDF(SESSION_SECRET, info='eex/session/v1')`; device-token key: `HKDF(DEVICE_TOKEN_SECRET, info='eex/device-token/v1')`. **Stream-token key: NOT HKDF-derived — uses raw UTF-8 bytes of `STREAM_TOKEN_SECRET` as the HMAC key.** Locked 2026-05-27 (ambitions-audit). Rationale: STREAM_TOKEN_SECRET is already domain-separated by `assertSecretsDistinct`; wiring HKDF post-M1.5 would either invalidate every 90-day playlist token in production OR require a tri-key verifier (raw-old + HKDF-new + legacy-SESSION fallback) for the transition window — more complexity than the cryptographic gain. `INFO_STREAM_TOKEN = 'eex/stream-token/v1'` remains exported from `keyDerivation.ts` as a reserved label for a future migration but is NOT applied by `signStreamToken` / `verifyStreamToken` today. Verifier on the session-cookie path: accept both old (raw SHA-256) and new (HKDF) derivations for a 30-day window (one cookie TTL), then drop the legacy path. Device-token path does not need a grace window because that secret is being introduced fresh in D13. Note: use full HKDF (Extract+Expand), not Extract-only — Extract-only without Expand loses domain separation via the `info` parameter. The info strings `eex/session/v1`, `eex/device-token/v1` are frozen wire values; do not rename post-ship without a verifier grace window. Cross-language: `tests/vectors/stream-token-canonical.json` `_meta.hmac_key_is = "raw_utf8_of_test_key"`. Rust `emerald-contracts::stream_token` MUST use `HMAC::new_from_slice(secret.as_bytes())`, not `HMAC::new_from_slice(&hkdf_derived)`. | `server/session.ts`, new `server/services/keyDerivation.ts` | 4h |
...
- [x] **Patch 8 — §5.1 `rid` escaping + pipe-ambiguity 400**: Added mandatory unconditional JSON escaping of `rid` and `sub` fields in the fixed-template serializer. Added grant-endpoint validation rejecting `streamId` containing `|` with `400 rid_invalid`. (Sources: rsrch-d1-canonical-json, rsrch-d3-ulid-vs-uuidv7)

- [x] **Patch 9 — §13.1 `_meta` block spec**: Expanded `_meta` block requirement to mandate `format_version: 1`, `test_key_hex` (inline test key in Wycheproof/NIST pattern), `reviewed_by`, and ISO 8601 `date`. Added MLDSA KAT incident (NIST PQC forum, 2024) as rationale. Added Swift consumption note (test-target bundle only). (Sources: rsrch-vectors-hygiene, rsrch-d1-canonical-json)

- [x] **Patch 10 — §11.1 `ANALYZE` + index name lock**: Added `ANALYZE iptv_title_link;` as the mandatory final statement of migration `0003_link_tombstones.sql` with SQLite query-planner rationale. Locked index name as `iptv_link_active_by_tmdb` with non-conformance note for alternate names. (Source: rsrch-d11-tombstones)

- [x] **Patch 11 — §16 v1.1 D-rows**: Added 7 new D-rows to §16 delta table: D2a-tsafe, D3-cache-wire, D5-busy-timeout, D11-analyze, D12-full (scope expansion), D13-gc, D14-probe-share, S15-beforeBreadcrumb. Updated effort totals. (Sources: rsrch-d2a-key-separation, rsrch-d3-ulid-vs-uuidv7, rsrch-d5-multi-db-sqlite, rsrch-d11-tombstones, rsrch-d12-playlist-tokens, rsrch-d13-device-tokens, rsrch-d14-reconcile-cascade, rsrch-s15-pii-scrubber)

Ratified 2026-05-27 (ambitions-audit synthesis). All 11 patches confirmed against code and inputs; contract may be tagged `m15-contract-v1.1.0-ratified`. The D-row implementation work tracked in §16 is independent of patch ratification — patches lock the spec text; D-rows track the code work.

## 19.2 M6 reserved enum values (forward-compatibility)

Implementations MUST accept (not reject as malformed) tokens that carry the following claim values. They are reserved for M6 Plex-Pass picks. No current code path mints them.

- **`role: 'guest'`** — reserved for M6 *Sharing / Invitations* pick (per-library guest access). Appears in: session cookie, device JWE, internal principal. Today's verifiers MUST treat unknown role values as `'user'` (least privilege) rather than 401-rejecting the token.
- **`kind: 'recording'`** — reserved for M6 *DVR for IPTV-to-disk* pick (catchup playback from local recordings). Appears in: stream-token kind enum (NOT in SessionKind — recordings are file-served, not concurrency-tracked the same way as live). Today's verifiers MUST treat unknown kind values as a hard reject (no fallback) since stream-kind drives access semantics.

Rationale: reserving these values now prevents a coordinated contract amendment + Keychain device-pair invalidation cascade once tokens are in production. Cost: one paragraph today vs. a multi-week rollout coordination at M6 spec-open. Reserved at 2026-05-27 per the ambitions-audit synthesis (m6-plex-pass-readiness, synth-top-blockers §5).
