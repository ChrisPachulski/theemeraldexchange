...
|---|----------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------|--------|
| 1 | Personalized fill         | % of refreshes that return ≥ TARGET_COUNT picks where ≥ 80% came from Claude (`source: 'personalized'`) — NOT `personalized_filled`, `_empty_trending_fallback`, or `trending_fallback`. The point of the section is personalization; falling back is the failure mode. | 2 — every short-of-target run silently fills from trending/discover and labels itself `personalized_filled`. Logs show this is common with non-trivial libraries. | ≥ 4 — `personalized` is the modal source for any account with library ≥ COLD_START_THRESHOLD and a valid key |
